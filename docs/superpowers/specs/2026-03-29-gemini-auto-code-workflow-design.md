...
3. **Execution:** The CLI is invoked in headless mode (`-p`) with the following prompt:
   "The status of this issue is 'Ready' in repository {owner/repo}. Issue #{number}: {title}
   {body}
...
2. **Permissions**: The workflow defines explicit write permissions for `contents` and `pull-requests`.
...
- Prompt construction uses `printf` to ensure safe handling of issue content.
...
## Success Criteria
- [x] A GitHub Action is successfully triggered by the "Ready" label.
- [x] The Gemini CLI starts correctly in headless mode.
- [x] The CLI creates a new branch and a Pull Request that addresses the issue content.
